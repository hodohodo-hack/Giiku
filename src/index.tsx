 ...
 */
class SimpleRenderer implements ICharacterRenderer {
  render(state: GiikuState) {
    // Determine the character's mood/look based on satiety
    let emoji = '👾';
    if (state.condition.satiety < 20) emoji = '💀 (Starving)';
    else if (state.condition.satiety < 50) emoji = '😧 (Hungry)';
    else if (state.condition.satiety > 90) emoji = '🤩 (Full)';
